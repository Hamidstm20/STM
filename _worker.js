...
// @ts-ignore
// https://github.com/AzadNet2024/AzadNet-Worker-Panel

import { connect } from 'cloudflare:sockets';

// How to generate your own UUID:
// https://www.uuidgenerator.net/
let userID = 'f06ac303-e626-451d-bcfc-ee825ee69398';
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (! await env.AzadNet.get('proxySettings')) {
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.AzadNet.get('secretKey');
                        let pwd = await env.AzadNet.get('pwd');


                        if (!pwd) {
                            await env.AzadNet.put('pwd', 'Ss3311@@');
                        }

                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.AzadNet.put('secretKey', secretKey);
                        }


                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.AzadNet.get('pwd');
                    ...
                    case '/panel/password':
                        ...
                        const oldPwd = await env.AzadNet.get('pwd');

                        if (newPwd === oldPwd) {
                            return new Response('Please enter a new Password!', { status: 400 });
                        }

                        await env.AzadNet.put('pwd', newPwd);
...
const getNormalConfigs = async (env, hostName, client) => {
    let vlessWsTls = '';
    const { cleanIPs } = await env.AzadNet.get("proxySettings", {type: 'json'});
...
const getFragmentConfigs = async (env, hostName, client) => {
    let Configs = [];
    let outbounds = [];
    let proxySettings = await env.AzadNet.get("proxySettings", {type: 'json'});
    const {
        remoteDNS, 
        localDNS, 
        lengthMin, 
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.AzadNet.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: ''}));
        }
    ...
    bestPing.remarks = '✅️AzadNet Frag - Best Ping 💥';
...
const getSingboxConfig = async (env, hostName) => {
    const {
        ...
        localDNS,
        cleanIPs
    } = await env.AzadNet.get("proxySettings", {type: 'json'});
...
const updateDataset = async (env, Settings) => {
    ...
    try {    
        await env.AzadNet.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
...
const Authenticate = async (request, env) => {
    const secretKey = await env.AzadNet.get('secretKey');
...
const renderHomePage = async (env, hostName, fragConfigs) => {
	...
	<body>
		<h1 style="text-align: center; color: #2980b9">AzadNet Panel <span style="font-size: smaller;">2.3</span> 💦</h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#AzadNet%20Normal', false)">
                                Copy Sub
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#AzadNet-Normal', false)">
                                Copy Sub
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#AzadNet-Normal', false)">
                                Copy Sub
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}#AzadNet Fragment', true)">
                                Copy Sub
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/AzadNet2024/AzadNet-Worker-Panel" target="_blank">Github</a>
...
const renderLoginPage = async () => {
    ...
    <body>
        <div class="container">
            <h1>AzadNet Panel <span style="font-size: smaller;">2.3</span> 💦</h1>
            <div class="form-container">
                <h2>User Login</h2>
                <form id="loginForm">
                    <div class="form-control">
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>KV Dataset is not properly set! Please refer to <a href="https://github.com/AzadNet2024/AzadNet-Worker-Panel/blob/main/README.md">documents</a></h2>
            </div>
        </div>
    </body>
